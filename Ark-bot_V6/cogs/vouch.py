...
class Vouch(commands.Cog):
    ...
    async def vouch(self, ctx):
        print("Comprobando nuevos vouch")
        while True:
            await asyncio.sleep(30)  # Cambiar "asyncio.sleep" a "await asyncio.sleep"
    ...
    async def on_message(self, message):
        if message.channel.id == config.VOUCH:
            ...
            with open(config.archivo_vote_vouch, 'a') as archivo:  # Cambiar 'w' a 'a' para agregar mensajes a la lista
